...
export class FilesService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.imageDirectory = this.configService.getOrThrow(
      EnvironmentVariables.IMAGE_FILES_DIR,
    );
  }
  ...
  async saveImageFile(file: Express.Multer.File): Promise<void> {
    const filePath = path.join(this.imageDirectory, file.originalname);
    await fs.writeFile(filePath, file.buffer);
  }
}
